...
const smiley = [
  circle,
  circle.map(
    (point) => new DOMPoint(point.x * 0.25 + 0.4, point.y * 0.25 - 0.3),
  ),
  circle.map(
    (point) => new DOMPoint(point.x * 0.25 - 0.4, point.y * 0.25 - 0.3),
  ),
...
const setAlpha = (hex: string, alpha: number) =>
  /^#?[0-9A-Fa-f]{8}$/.test(hex)
    ? `#${hex.replace('#', '').slice(0, 6)}${Math.round(alpha * 255)
        .toString(16)
        .padStart(2, '0')}`
    : (() => {
...
export class SvgGraphicsViewComponent {
  ...
  vm = computed(() =>
    [...this.matrices(), new DOMMatrix()].reduceRight(
      (acc, matrix, matrixIndex) => {
        ...
        const pictureAlpha =
          this.hoveredPicture() === matrixIndex
            ? 0.8
            : pictureSelected
              ? 0.1
              : 0.2;
        ...
        if (matrixIndex !== lastIndex) {
          ...
          const color = setAlpha(
            this.transformationColors()[matrixIndex],
            intensity,
          );
